...
in
{
  ...
  config = lib.mkIf config.myConfig.modules.claude.enable {
    ...
    home-manager.users.${username} = ({ config, ... }: {
      ...
      home.activation.syncGafferPlugin = config.lib.dag.entryAfter [ "writeBoundary" "linkClaudeSettings" "installCaveman" ] "if ! run env CLAUDE_BIN=${claudePackage}/bin/claude GIT_BIN=${pkgs.git}/bin/git JQ_BIN=${pkgs.jq}/bin/jq TIMEOUT_BIN=${pkgs.coreutils}/bin/timeout FLOCK_BIN=${pkgs.util-linux}/bin/flock MV_BIN=${pkgs.coreutils}/bin/mv SLEEP_BIN=${pkgs.coreutils}/bin/sleep WC_BIN=${pkgs.coreutils}/bin/wc PS_BIN=${psBin} GAFFER_HOME=$HOME/code/gaffer GAFFER_SOURCE=$HOME/.local/state/north/gaffer-plugin-source GAFFER_REV=${gafferRev} ${gafferPluginSync}; then\n  echo \"warning: Gaffer Claude plugin sync failed; agent-config-check --local will report the drift\" >&2\nfi\n";
      home.activation.registerMcpServers = config.lib.dag.entryAfter [ "writeBoundary" ] "CLAUDE=${claudePackage}/bin/claude\nLIFE=$HOME/.local/state/north\nWANT_FRAM_LOG=$LIFE/coordination.log\nWANT_FRAM_TELEMETRY_LOG=$LIFE/telemetry.log\nWANT_FRAM_THREADS=$LIFE/threads\nWANT_NORTH_PORT=7977\nFRAM_CFG=$($CLAUDE mcp get fram 2>/dev/null || true)\nif ! grep -Fq \"FRAM_LOG=$WANT_FRAM_LOG\" <<<\"$FRAM_CFG\" || ! grep -Fq \"FRAM_TELEMETRY_LOG=$WANT_FRAM_TELEMETRY_LOG\" <<<\"$FRAM_CFG\" || ! grep -Fq \"FRAM_THREADS=$WANT_FRAM_THREADS\" <<<\"$FRAM_CFG\"; then\n  run timeout 30 $CLAUDE mcp remove fram -s user >/dev/null 2>&1 || true\n  run timeout 30 $CLAUDE mcp add fram -s user -e FRAM_LOG=$WANT_FRAM_LOG -e FRAM_TELEMETRY_LOG=$WANT_FRAM_TELEMETRY_LOG -e FRAM_THREADS=$WANT_FRAM_THREADS -- $HOME/code/fram/bin/fram-mcp || true\nfi\nNORTH_MCP_BIN=\${NORTH_CHECKOUT:-$HOME/code/north-routing-robustness-landing}/bin/north-mcp\nNORTH_CFG=$($CLAUDE mcp get north 2>/dev/null || true)\nif ! grep -Fq \"Command: $NORTH_MCP_BIN\" <<<\"$NORTH_CFG\" || ! grep -Fq \"FRAM_LOG=$WANT_FRAM_LOG\" <<<\"$NORTH_CFG\" || ! grep -Fq \"FRAM_TELEMETRY_LOG=$WANT_FRAM_TELEMETRY_LOG\" <<<\"$NORTH_CFG\" || ! grep -Fq \"FRAM_THREADS=$WANT_FRAM_THREADS\" <<<\"$NORTH_CFG\" || ! grep -Fq \"NORTH_PORT=$WANT_NORTH_PORT\" <<<\"$NORTH_CFG\"; then\n  run timeout 30 $CLAUDE mcp remove north -s user >/dev/null 2>&1 || true\n  run timeout 30 $CLAUDE mcp add north -s user -e FRAM_LOG=$WANT_FRAM_LOG -e FRAM_TELEMETRY_LOG=$WANT_FRAM_TELEMETRY_LOG -e FRAM_THREADS=$WANT_FRAM_THREADS -e NORTH_PORT=$WANT_NORTH_PORT -- $NORTH_MCP_BIN || true\nfi\nif ! $CLAUDE mcp get linear-mcp-msa-new >/dev/null 2>&1; then\n  run timeout 30 $CLAUDE mcp add --transport http linear-mcp-msa-new https://mcp.linear.app/mcp -s user || true\nfi\n";
    });
  };
}
