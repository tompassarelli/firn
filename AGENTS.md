...
Agents QUEUE rebuilds and never fire them: `north rebuild request --why "<reason>"` records one durable ask and returns at once (`--urgent "<why>"` when it cannot wait — counted, never refused), and the reactor's window owner coalesces every open ask into ONE coordinated rebuild. Direct `firn rebuild` / `firn-rebuild-coordinated` are DENIED for agent tool calls. A rebuild still builds a **commit snapshot** (`git+file?rev=HEAD`), never the working tree — no session's uncommitted state can block it or leak into a generation, so there is no "dirty tree" precondition to check. The one rule that is yours: **commit your own changes first**, or they won't be in the build when the window fires (the pipeline prints what it excluded). The pipeline itself regenerates + validates the snapshot, self-heals stale committed `.nix`, and promotes each local input's exact committed local `refs/heads/main` even when its active checkout is dirty or off-main. Feature HEADs are excluded; missing, rewound, or divergent local main refs hold their verified pins. The exact verified closure is what switches. Never raw `nh`/nixos-rebuild`, never `firn update` (input bumps are the USER's). Build-only verification: `nix build --no-link`. Only verify whiterabbit; skip thinkpad-x1e.
