#!/usr/bin/env bash
# PreToolUse agent-spawn-guard — enforcement half of the DISPATCH knob (see `knobs`).
# ============================================================================
...
#
# State:       ~/.claude/knobs.state  (flip via `knobs dispatch <mode>`)
...
MODE=$(grep -E '^dispatch=' "$HOME/.claude/knobs.state" 2>/dev/null | tail -1 | cut -d= -f2-)
MODE="${MODE:-tern}"
[ "$MODE" = "native" ] && exit 0
...
recipe = (
    ...
    "Bypass deliberately: knobs dispatch warn|native (or /knobs)."
)
