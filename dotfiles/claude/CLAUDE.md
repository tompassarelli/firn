...
fast and cost tokens. Take a beat before writing one. A good comment is a micro
dev-log: it encodes INTENTION, a trade-off that was made, or a breadcrumb for a
path NOT taken and why (time pressure, etc.) — in extreme terseness. A bad comment
restates the code or echoes a plan from somewhere upstream (the rot-prone
copy-paste smell). If the comment doesn't say something the code can't, drop it.
(Longer-term: link intention to a claim/thread instead of inlining it.)
