...
agent here. Gaffer describes the work; North resolves the runtime.
...
- **Atomic** — one terminal objective with bounded scope, known inputs and
  outputs, and one verification path. Select exactly one worker-topology Gaffer
  ...
  a failure to use Gaffer. Never select `director` or orchestrator topology for
  atomic work.
...
supported explicit override contract says otherwise. Semantic Gaffer options
...
worker-spawns-verifier exception: Gaffer worker topology never spawns or
