...
Agent Machinery owns the provider-independent eight-field run design. North
owns concrete admission, provider/account allocation, lifecycle, identity, and
...
the current session. Do not select a concrete model or native provider agent
here.
...
- **Atomic** — one terminal objective with bounded scope, known inputs and
  outputs, and one verification path. Select exactly one worker-topology Agent
  Machinery role. Use an unchanged canonical preset when it fits; pass semantic axis
  ...
  a failure to use Agent Machinery. Never select `director` or orchestrator topology for
  atomic work.
...
supported explicit override contract says otherwise. Semantic Agent Machinery options
...
worker-spawns-verifier exception: Agent Machinery worker topology never spawns or
