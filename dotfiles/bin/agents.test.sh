...
if [ "${1:-}" = "--executive-report-skill" ]; then
  ...
  chk "frontmatter names all four report triggers" "1" "$(grep -Fq '“executive report,”' "$skill_root/SKILL.md" && grep -Fq '“weekly' "$skill_root/SKILL.md" && grep -Fq '“workstream brief,”' "$skill_root/SKILL.md" && grep -Fq '“milestone retrospective”' "$skill_root/SKILL.md" && echo 1 || echo 0)"
  ...
  chk "trivial work declines the retrospective form" "1" "$(grep -Fq 'decline the form plainly and give at most a one-line' "$skill_root/SKILL.md" && echo 1 || echo 0)"
  ...
  chk "root reuses the canonical commander card" "1" "$(grep -Fq 'Do not define a second card schema' "$skill_root/SKILL.md" && echo 1 || echo 0)"
  chk "single-front reports avoid portfolio collection" "1" "$(grep -Fq 'do not collect portfolio-wide state' "$skill_root/SKILL.md" && echo 1 || echo 0)"
  chk "authority stages remain distinct" "1" "$(grep -Fq 'candidate “done,”' "$skill_root/SKILL.md" && echo 1 || echo 0)"
  chk "OpenAI metadata invokes the requested report type" "1" "$(grep -Fq 'Use $executive-report to produce the requested concise report type' "$skill_root/agents/openai.yaml" && echo 1 || echo 0)"
  ag status > /dev/null
