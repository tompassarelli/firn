...
**`firn update` moves every input forward; `firn rebuild` promotes only committed local development inputs.** On rebuild, Firn *plans* pin moves for `~/code/beagle`, `~/code/fram`, and `~/code/north` (read-only — the lock is never provisionally mutated): a checkout advances its pin only when its tracked source is committed and it sits on `main`; anything else — dirty tracked files (another session's WIP), a feature branch — **holds its already-verified pin** for this run with a printed notice and promotes on a later rebuild once clean. Planned moves ride the snapshot build as `--override-input` flags; the whole rebuild evaluates `git+file://<repo>?rev=HEAD`, so no working tree (this repo's or any input's) can leak in or block. Only after the host closure builds is `flake.lock` re-pointed and mechanically committed ("refresh verified local inputs"); every non-promotable state at that point — a foreign lock edit, an input that moved since planning, a hook failure — defers with a notice and exit 0, because the switched generation already carries the verified revs. `--skip-checks` builds the HEAD snapshot with the committed lock and never promotes pins. Untracked editor/daemon state is ignored, and remote inputs such as nixpkgs remain pinned.
