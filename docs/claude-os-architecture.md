# Claude System — Architecture Map
...
> here means the system actually moved.
...
- **hooks** (Claude Code fires these at lifecycle points; `⟨src⟩` = settings.json or the plugin that contributes it):
  ...
  - `PreToolUse` → `claim-canonical-guard.sh` ⟨settings⟩, `firnos-guard.sh` ⟨settings⟩, `fleet-protocol-guard.sh` ⟨settings⟩, `firnos-guard.sh` ⟨settings⟩
